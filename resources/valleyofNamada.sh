...
- current namada node version: ${CYAN}v1.0.0 - v1.1.1 - v1.1.5 - v101.1.7 - v201.0.7${RESET}
- current cometbft version: ${CYAN}v0.37.15${RESET}
- current namada indexer version: ${CYAN}v4.1.0${RESET}
...
function deploy_validator_node() {
    ...
    echo -e "  ${BLUE}https://github.com/hubofvalley/Valley-of-Namada-Mainnet/blob/main//resources/namada_validator_node_install_mainnet.sh${RESET}"

    echo -e "\n${YELLOW}2. SYSTEM IMPACT:${RESET}"
    echo -e "${GREEN}New Services:${RESET}"
    echo -e "  • ${CYAN}namadad.service${RESET} (Consensus Client)"
    
    echo -e "\n${RED}Existing Services to be Replaced:${RESET}"
    echo -e "  • ${CYAN}namadad${RESET}"
    ...
    echo -e "  • ${CYAN}$HOME/.local/share/namada${RESET}"
...
function show_validator_node_status() {
    ...
    if (( block_difference < 0 )); then
        echo -e "${GREEN}Note:${RESET} A negative value is normal - this means Grand Valley's Namada Mainnet RPC block height is currently behind your node's height"
    fi
...
function install_namada_app() {
    ...
    wget https://github.com/anoma/namada/releases/download/v201.0.7/namada-v201.0.7-Linux-x86_64.tar.gz
    tar -xvf namada-v201.0.7-Linux-x86_64.tar.gz
    cd namada-v201.0.7-Linux-x86_64
...
function show_guidelines() {
    ...
    echo "   - Install Namada App (v201.0.7) to execute transactions without running a node"
...
function menu() {
    ...
    echo -e "${GREEN}6. Install the Namada App (v201.0.7) only to execute transactions without running a node${RESET}"
